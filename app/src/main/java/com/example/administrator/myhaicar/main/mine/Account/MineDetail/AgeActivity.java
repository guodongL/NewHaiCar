...
public class AgeActivity extends SwipeBackToolBarActivity {
    ...
    private void initClick() {
       ...
       textView_save.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                OkHttpUtils.post().url(UrlConfig.Path.BondID_URL).addParams("acts","usr_age").addParams("usr_id",usr_id).addParams("upcont",editText_age.getText()+"").build().execute(new StringCallback() {
                    @Override
                    ...
                    public void onResponse(String s) {
                        ...
                        if (treatedok.equals("1")) {
                            PreferencesUtils.putString(mContext, "usr_ages", editText_age.getText() + "");
                            finish();
                        }
                    }
                });
